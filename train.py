        ...
        for line in fin:
            ...
            for g in groups:  # type: str
                ...
                if g.isalpha():
                    ...
                    _symbol[size][g] += 1
            _structures[s] += 1
    return _structures, list(_letter), _digits, _symbol
...
def write2disk(_structures: dict, _letter: list, _digits: dict, _symbol: dict, model_folder: str,
    ...
    for l, s in _symbol.items():
        ...
        f_symbol.flush()
        f_symbol.close()
    f_letter = open(path.join(model_folder, "dictionaries.txt"), "w")
    f_letter.writelines([f"{l}\n" for l in _letter])
    f_letter.flush()
    f_letter.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-x", "--extra-dict", required=False,
                        help="you may supply some words to enrich the letter counted from pwd-set")
    args = parser.parse_args()
    structures, letter, digits, symbol = read_pwd_set(args.pwd_set)
    write2disk(structures, letter, digits, symbol, args.model, args.grammar, args.digits, args.symbol)
    print("Done!")
